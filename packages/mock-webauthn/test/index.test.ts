...
import { type AttestationResult, KeyType } from "../src/index.js";

test("EC2 E2E", async ({ expect }) => {
  const ec2keys = await mockWebauthn.EC2SigningKey.generate().then(
    async (ec2) => ec2.export()
  );
  ...
  const attestationResponse = await mockWebauthn.createAttestationResponse(
    rp,
    auth,
    cred,
    {
  ...
  const verification = await verifyRegistrationResponse({
    response: JSON.parse(attestationResponse),
  ...
  const assertionResponse = await mockWebauthn.createAssertionResponse(
    rp,
    auth,
    cred,
    {
  ...
  const authVerification = await verifyAuthenticationResponse({
    response: JSON.parse(assertionResponse),
    expectedChallenge: mockWebauthn.base64.encodeUrlSafe(challengeEnc),
    authenticator: {
